...
- `01_공통규칙/` — 3인 공통 문서 (테이블 정의서, 서비스 방향성, 기능 정의서, 권한 매트릭스, 파일명 고정 규칙, 작업자별 착수 프롬프트)
  - 가장 먼저 읽어야 할 문서: `01_공통규칙/shopdb2_백엔드_파일명_고정규칙_v0.3_2026.09.14.md`

## 브랜치 전략

- `main`: 공통 문서/DB 백업 등 공유 자산만 관리, 직접 push 금지 (PR로만 병합)
- `dev/A`, `dev/B`, `dev/C`: 담당자별 작업 브랜치 (담당 도메인은 `01_공통규칙` 문서 참고)

## 시작하기 전에

각 작업자는 `01_공통규칙/작업자별_착수프롬프트_0.1v_260914_15.09.md`에서 본인 담당 섹션의 프롬프트를 복사해 AI 세션 첫 메시지로 사용하세요.
