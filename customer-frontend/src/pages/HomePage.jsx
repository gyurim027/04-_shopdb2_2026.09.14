    ...
    <>
          ...
          {featured ? (
            <div className="product-ad-shell">
              <div className="product-ad-copy">
                ...
                <div className="product-ad-price">
                  ...
                  <div>
                    {Number(featured.regular_price) > Number(featured.sale_price) && (
                      <span>{money(featured.regular_price)}원</span>
                    )}
                ...
                <div className="product-ad-actions">
                  <Link to={`/products/${featured.product_id}`} className="btn btn-primary">
                    상품 보러가기 <ArrowRight size={18} />
                  </Link>
              ...
              <Link to={`/products/${featured.product_id}`} className="product-ad-visual" aria-label={`${featured.product_name} 상세보기`}>
                {adImage ? (
                  <img src={adImage} alt={featured.product_name} />
                ) : (
                  <div className="product-ad-placeholder">
                    <ShoppingBag size={74} />
                    <span>SHOPDB</span>
              ...
              {adProducts.length > 1 && (
                <>
                  <button type="button" className="ad-arrow ad-arrow-left" onClick={() => moveAd(-1)} aria-label="이전 광고">
                    <ChevronLeft />
                  </button>
                  <button type="button" className="ad-arrow ad-arrow-right" onClick={() => moveAd(1)} aria-label="다음 광고">
                    <ChevronRight />
                  </button>
            ...
            <div className="product-ad-shell product-ad-loading">
              <div>
                ...
                <h1>오늘의 상품을 준비하고 있어요.</h1>
                <p>판매 중인 상품을 불러오면 이 영역에 상품 광고가 표시됩니다.</p>
                <Link to="/products" className="btn btn-primary">상품 전체보기 <ArrowRight size={18} /></Link>
              </div>
      ...
      <section className="container section">
        ...
        <div className="category-chips">
          ...
          {categories.slice(0, 10).map((c) => (
            <Link
              key={c.category_id}
              to={`/products?category=${c.category_id}`}
      ...
      <section className="container section">
        <div className="section-head">
          <div>
            ...
            <h2>지금 둘러볼 상품</h2>
            <p>현재 판매 중인 상품을 빠르게 확인하세요.</p>
          </div>
          <Link to="/products" className="text-link">더보기 <ArrowRight size={16} /></Link>
        </div>
        {error ? (
          <div className="notice error">백엔드 연결을 확인해주세요: {error}</div>
        ) : (
      ...
      <section className="container service-banner">
        <div>
          <span>고객센터</span>
          <h2>궁금한 점이 있으신가요?</h2>
          <p>1:1 문의와 AI 챗봇으로 빠르게 해결해보세요.</p>
        </div>
        <div className="service-actions">
          <Link to="/support" className="btn btn-dark">1:1 문의</Link>
          <Link to="/ai" className="btn btn-outline">AI 챗봇</Link>
        </div>
      </section>
    </>
  )
