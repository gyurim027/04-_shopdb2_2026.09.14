...
function readStoredOrders() {
  try {
    const parsed = JSON.parse(
      sessionStorage.getItem('shopdb2_cart_checkout_orders') || '[]',
    )
...
export default function CartCheckoutPage() {
  ...
  const seedOrders =
    location.state?.createdOrders || readStoredOrders()
    ...
    try {
      const details = await Promise.all(
        seedOrders.map(async (seed) => {
          const order = await customerApi.getOrder(
            seed.order_id,
          )
  ...
  const payOne = async (order) => {
    ...
    try {
      const request =
        await customerApi.requestPayment({
          order_id: Number(order.order_id),
          pg_provider: 'TOSS',
          payment_method: 'CARD',
        })

      await customerApi.approvePayment(
        request.payment_id,
        {
  ...
  const payAll = async () => {
    ...
    if (
      !window.confirm(
        `미결제 주문 ${unpaidOrders.length}건을 순서대로 Mock 카드결제할까요?`,
      )
    ) {
      return
    }
        ...
        <aside className="cart-checkout-summary">
          ...
          {unpaidOrders.length ? (
            ...
            >
              ...
              {payingAll
                ? '순차 결제 중...'
                : `미결제 ${unpaidOrders.length}건 전체 Mock 결제`}
            </button>
          ) : (
            <button
              type="button"
